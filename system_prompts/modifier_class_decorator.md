...
Rewrite the entire python file provided.  

[CRITICAL] Retain the original code as-is, only augment the code by adding one **import** and the **class decorator** as described. IT IS CRITICALLY IMPORTANT TO RETAIN THE ORIGINAL APPLICATION CODE.

- **IMPORT**: add import for the class decorator: `from dagrr_core.common.etl_metadata.transformations import etl_step`
- **CLASS DECORATOR**: add the `etl_step` class decorator to the class with a `modify` function.  
...
```python
   ...
  # existing imports AGENT DO NOT CHANGE OR REMOVE
  import numpy
  from typing import List

  # This was INSERTED by the Agent
  from dagrr_core.common.etl_metada.transformations import etl_step
...
# [CRITICAL] Nothing from here to the end of the file is to be changed by the Agent
class AddCustomerAgeColumn:
    ...
    @property
    def requires(self) -> List[str]:
